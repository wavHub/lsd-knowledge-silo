#!/usr/bin/env python3
"""Run resumable full LightRAG extraction for 5 knowledge silos.

- Picks model from benchmark output unless overridden.
- Segments by headings/chapter sections (no raw file insertion).
- Uses Modal + Ollama model cache volume for remote execution.
- Writes merged collection GraphML to data/lightrag-ref.
- Persists nodes/edges/chunks/runs to Supabase if schema is available.
"""
...
APP_NAME = "lsd-lightrag-full-pass"
REMOTE_WORK_ROOT = "/tmp/lightrag-full"
EMBED_MODEL = "nomic-embed-text"
FULL_GPU = os.getenv("MODAL_FULL_GPU", "H100").strip() or "H100"
...
def build_full_segments(source_root: Path) -> list[dict[str, Any]]:
    segments: list[dict[str, Any]] = []
    for file_name, collection in FILE_COLLECTION_MAP.items():
        path = source_root / file_name
        if not path.exists():
            continue
        division = COLLECTION_TO_DIVISION[collection]
        ...
        for section_idx, (heading, section_text) in enumerate(sections, start=1):
            for sub_idx, sub_text in enumerate(_subchunk_text(section_text), start=1):
                segment_id = hashlib.sha1(
                    f"{collection}|{file_name}|{section_idx}|{sub_idx}|{sub_text[:200]}".encode("utf-8")
                ).hexdigest()[:16]
                segments.append(
                    {
...
def persist_to_supabase(
    *,
    ...
    required_tables = ["graph_nodes", "graph_edges", "knowledge_chunks", "extraction_runs"]
    ...
    for seg in segments:
        seg_result = next((x for x in results if x.get("segment_id") == seg.get("segment_id")), None)
        status = str(seg_result.get("status", "pending")) if seg_result else "pending"
        chunk_rows.append(
            {
                "id": str(seg["segment_id"]),
                "division": str(seg["division"]),
                "collection": str(seg["collection"]),
                "book": str(seg["file_name"]),
                "chapter": str(seg["heading"]),
                "section": str(seg["heading"]),
                "chunk_index": int(seg.get("section_index", 0) or 0),
                "char_count": int(seg.get("char_count", 0) or 0),
                "token_est": int(seg.get("token_est", 0) or 0),
                "chunk_hash": hashlib.sha1(str(seg["segment_id"]).encode("utf-8")).hexdigest(),
                "storage_path": None,
                "status": "extracted" if status == "success" else "failed",
                "extraction_model": model_name,
                "nodes_extracted": int(seg_result.get("nodes_extracted", 0) or 0) if seg_result else 0,
                "edges_extracted": int(seg_result.get("edges_extracted", 0) or 0) if seg_result else 0,
                "processing_started_at": _utcnow_iso(),
                "processing_completed_at": _utcnow_iso(),
                "error_message": str(seg_result.get("error_message", "")) if seg_result else None,
            }
        )

    _upsert_rows(base_url, headers, "extraction_runs", extraction_runs_rows)
    _upsert_rows(base_url, headers, "knowledge_chunks", chunk_rows)
...
def write_graphml_outputs(results: list[dict[str, Any]]) -> dict[str, dict[str, int]]:
    ...
    output_root = REPO_ROOT / "data" / "lightrag-ref"
...
def main(
    ...
    selected_model: str = "",
    checkpoint_path: str = str(REPO_ROOT / "docs" / "LIGHTRAG-FULL-PASS.checkpoint.json"),
    output_json: str = str(REPO_ROOT / "docs" / "LIGHTRAG-FULL-RUN-RESULT.json"),
    max_segments_per_collection: int = 0,
    rerun_failed: bool = True,
    skip_supabase: bool = False,
) -> None:
    source_root = Path(source_dir).resolve()
    segments = build_full_segments(source_root)
    if not segments:
        raise RuntimeError("No full-pass segments built from source markdown.")
    ...
    print(f"[full-pass] selected model: {model_name}")
    ...
    results: list[dict[str, Any]] = []
    run_id = f"full-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}-{uuid.uuid4().hex[:6]}"
    ...
    for seg in segments:
        ...
        print(
            f"[full-pass] model={model_name} collection={seg['collection']} "
        ...
        print(
            f"[full-pass] complete collection={result['collection']} status={result['status']} "
